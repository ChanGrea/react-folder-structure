...
test('serviceAction', () => {
  const action = (suffix, service) => ({
    type: `SOCIAL_LOGIN_${suffix}`,
    payload: { service },
  })
  expect(
    sagas.serviceAction('REQUEST', 'facebook')(action('REQUEST', 'facebook')),
  ).toBe(true)
  expect(
    sagas.serviceAction('REQUEST', 'facebook')(action('PREPARE', 'facebook')),
  ).toBe(false)
  expect(
    sagas.serviceAction('REQUEST', 'facebook')(action('REQUEST', 'google')),
  ).toBe(false)
})
...
describe('loginFacebook', () => {
  ...
  it('calls success', () => {
    ...
    expect(generator.next().value).toEqual(
      call(sagas.promises.fbLogin, { scope: 'public_profile' }),
    )
    expect(generator.next().value).toEqual(
      call(sagas.promises.fbGetMe, { fields: 'id,name' }),
    )
    expect(generator.next({ id: '123', name: 'name' }).value).toEqual(
      put(
  ...
  it('calls failure', () => {
    ...
    expect(generator.throw('test').value).toEqual(
      put(actions.socialLoginFailure('test', request)),
    )
  })
})
...
describe('prepareFacebook', () => {
  ...
  it('calls success', () => {
    ...
    expect(generator.next().value).toEqual(
      call(loadScript, '//connect.facebook.net/en_US/sdk.js'),
    )
    expect(generator.next().value).toEqual(
      call([window.FB, window.FB.init], {
        appId: 'test',
        version: 'v2.8',
        foo: 'bar',
      }),
...
test('watchSocialLoginFacebook', () => {
  ...
  expect(generator.next({ payload }).value).toEqual(
    call(sagas.prepareFacebook, payload),
  )
  generator.next()
  expect(generator.next({ payload }).value).toEqual(
    call(sagas.loginFacebook, payload),
  )
})
...
describe('loginGoogle', () => {
  ...
  it('calls success', () => {
    ...
    expect(generator.next(user).value).toEqual(
      call([user, user.getBasicProfile]),
    )
    ...
    expect(generator.next('imageUrl').value).toEqual(
      put(
        actions.socialLoginSuccess(
          { name: 'name', picture: 'imageUrl' },
          request,
        ),
      ),
    )
...
describe('prepareGoogle', () => {
  ...
  it('calls success', () => {
    ...
    expect(generator.next().value).toEqual(
      call(loadScript, '//apis.google.com/js/platform.js'),
    )
  ...
  it('calls failure', () => {
    ...
    expect(generator.throw('test').value).toEqual(
      put(actions.socialLoginFailure('test', { service: 'google', ...payload })),
    )
  })
})
...
test('watchSocialLoginGoogle', () => {
  ...
  generator.next()
  expect(generator.next({ payload }).value).toEqual(
    call(sagas.prepareGoogle, payload),
  )
  generator.next()
  expect(generator.next({ payload }).value).toEqual(
    call(sagas.loginGoogle, payload),
  )
})
